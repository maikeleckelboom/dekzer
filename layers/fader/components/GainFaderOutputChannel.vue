...
const { dBMin = -48, dBMax = 0, segments = 40, orient = 'vertical' } = defineProps<SegmentMeterProps>()
...
function interpolateColor(start: string, end: string, normalized: number): string {
	const interpolateChannel = (startHex: string, endHex: string): number =>
		parseInt(startHex, 16) + Math.round(normalized * (parseInt(endHex, 16) - parseInt(startHex, 16)))
...
function getClosestColorKey(value: number): number {
	return Array.from(colorMap.keys()).reduce((a, b) => (Math.abs(b - value) < Math.abs(a - value) ? b : a))
}
...
<template>
	<div class="flex flex-col size-fit">
		<div
			...
			class="w-[14px] h-[3px] bg-muted/75 mb-[2px] last:mb-0" />
	</div>
</template>
