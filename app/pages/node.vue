...
const deckGainNode = shallowRef<GainNode>()
const deckVolumeGainNode = shallowRef<GainNode>()

const deck1VolumeValue = shallowRef<number>(DECK_GAIN_DEFAULT)
const deckVolumeValue = shallowRef<number>(VOLUME_FADER_DEFAULT)
...
onMounted(() => {
  ...
  deckGainNode.value = audioContext.createGain()

  masterGainNode.value.gain.value = masterGainValue.value
  deckGainNode.value.gain.value = deckVolumeValue.value
  ...
  // Connect both decks
  connectDeck(deck1Audio.value, deckGainNode.value, deckVolumeGainNode.value)
...
function onGainChange(event: InputEvent) {
  ...
  switch (parseInt(el.dataset.deck)) {
    case 1:
      const deck1Gain = unref(deckGainNode)!
      deckVolumeValue.value = inputValue
...
function resetDeckGain(deckNum: number) {
  switch (deckNum) {
    case 1:
      const deck1Gain = unref(deckGainNode)!
      deckVolumeValue.value = DECK_GAIN_DEFAULT
...
function onDeckVolumeChange(event: InputEvent) {
  ...
  deckGainValue.value = value
  deckVolumeGainNode.value!.gain.value = value
}

const masterGainDisplay = computed(() => faderToDB(masterGainValue.value, -12, 12).toFixed())
const deckGainDisplay = computed(() => faderToDB(deckVolumeValue.value, -24, 24).toFixed())
</script>
          ...
          <section class="grid grid-cols-2 gap-x-2 gap-y-4 border-2 bg-gray-400/10 p-2">
            ...
            <section
              ...
              <div class="flex flex-col items-center">
                <Button
                  :disabled="deckVolumeValue === DECK_GAIN_DEFAULT"
                  class="mr-auto"
                  size="xs"
                  variant="outline"
            ...
            <section
              ...
              <div class="grid place-items-center gap-4">
                ...
                <div class="flex w-full flex-col items-start">
                  <Button
                    :disabled="deckGainValue === VOLUME_FADER_DEFAULT"
                    size="xs"
                    variant="outline"
                    @click="deckGainValue = VOLUME_FADER_DEFAULT">
                    Reset
                  </Button>
                </div>
              </div>
